...
public class AspectLogging {
    ...
    //("execution(* здесь !!! все что нужно !!! прописывать только через Copy Reference)
    // если у нас много таких методов тогда: (..) ")
    public void saveProduct(){}

    @Before("saveProduct()")
    public void beforeSavingProduct(JoinPoint joinPoint){
        // Advice - тот код, который и будет внедряться в основной код,
...
//    }
    ...
    public void afterThrowingAnExceptionWhileGettingProductById(Exception e){
        logger.info("Method getById of the class ProductServiceImpl threw an exception while getting product: " +
    ...
    public Object aroundGettingAllProducts(ProceedingJoinPoint joinPoint){
        ...
        try {
            ...
            logger.error("Method getAll of class ProductServiceImpl threw exeption: {}", e.getMessage());
        }

        logger.info("Method getAll of the class ProductServiceImpl finishshed its work");
        return result;
    }
}
